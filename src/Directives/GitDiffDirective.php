...
final class GitDiffDirective extends AbstractDirective
{
    ...
    private function askForRecipeExtensions(): array
    {
        ...
        if (empty($recipeNames)) {
            $this->console->alertWarning('⚠️  No recipes configured');
    ...
    private function createWorkSummary(): void
    {
        ...
        if (empty($name)) {
            $this->console->alertWarning('⚠️  No name provided, skipping summary creation');
        ...
        if (empty($content)) {
            $this->console->alertWarning('⚠️  No content provided, skipping summary creation');
