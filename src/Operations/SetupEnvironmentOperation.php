    ...
    public static function handle(SshService $sshService, string $remotePath, bool $dryRun = false, ?Console $console = null): DeploymentResultRecord
    {
        ...
        if ($dryRun) {
            if ($console) {
                ...
                $console->line("   test -f {$remotePath}/.env || cp {$remotePath}/.env.production {$remotePath}/.env");
            ...
            return DeploymentResultRecord::from([
                ...
                'commands_executed' => ['check .env', 'copy .env.production', 'copy .env.example', 'key:generate'],
            ]);
        }
        ...
        if (! str_contains($checkEnvResult->output, 'EXISTS')) {
            ...
            // ✅ Priorité 1: .env.production déjà présent sur le serveur
